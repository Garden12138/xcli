...
为非交互 `run` 或 `workflow run` 添加 `--detach`，即可在独立进程会话中启动任务并立即返回。Agent 选择、cwd、环境配置、结构化解析、usage 采集、工作流并行度和原生参数均与前台运行一致：
...
后台执行的 `--json` 返回初始 Job 对象，而不是最终结果。Job JSON 包含类型、ID、Agent 或 workflow、状态、worker PID、cwd、时间和私有日志路径。workflow Job 还会展示并行度与实时步骤状态；任务结束后增加退出码、session ID 和可用的 usage。`jobs list` 只列出后台记录，`jobs show` 始终以 JSON 返回单个 Job。

每个任务都会保存权限为 `0600` 的私有归一化日志。原生 stderr 实时追加；普通 generic run 的 stdout 实时追加；结构化 run 只追加归一化最终文本。workflow 日志包含进度、原生 stderr 和最终步骤摘要，步骤输出只有在显式启用输出记录时才会保存。原始结构化事件不会进入归一化日志。日志可能包含源代码、诊断或密钥，xcli 不提供自动保留策略。
...
`jobs wait` 最终返回 Job 的退出码；等待超时返回 124，Ctrl+C 返回 130，二者都不会停止任务。`jobs delete` 经确认后删除一个终态 Job。`jobs prune` 必须显式给出时间范围，按固定顺序预览并支持 `--dry-run`；自动化需传 `--yes`。两者都拒绝删除运行中任务，xcli 也不会自动执行保留期清理。

后台启动输入通过匿名管道交给 worker；prompt、workflow 定义、变量值和原生参数不会进入 worker argv 或运行元数据。后台任务可以在启动终端退出后继续，但不能跨机器重启恢复。目前仍不包含 daemon、任务 restart/attach、后台 resume 或交互后台模式。
...
使用 `workflow run <file> --detach` 可以让同一套调度器作为后台 Job 运行。执行期间 `jobs show` 会展示 pending/running 步骤，`jobs logs --follow` 会持续输出进度直到 workflow 进入终态。
...
CAP、自动/持续 MCP 对账、项目自动发现、厂商高级 MCP 字段、守护进程、后台 resume 与交互会话、任务 restart/attach、Windows ConPTY 和 Web UI 仍继续延后。
